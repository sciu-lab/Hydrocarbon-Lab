...
async function copyVisibleName(value: string) {
  try {
    ...
    // Some embedded browsers require the selection-based clipboard path.
  }
...
/**
 * An unrepresented heteroatom makes the local name incomplete. This coverage
 * check also protects the dock while a verified external context is loading.
 */
...
/** The canvas context is already checked against the current graph by its resolver. */
export function pubChemIdentityForNomenclature(
  currentSmiles: string,
  selectedIdentity: PubChemStructureIdentity | null,
  context: CompoundContext | null,
): PubChemStructureIdentity | null {
  if (selectedIdentity?.smiles === currentSmiles) return selectedIdentity;
  if (context?.identityKey !== compoundIdentityKey({ canonicalSmiles: currentSmiles }) || !context.pubchem) return null;
  return {
    cid: context.pubchem.cid,
    ...(context.pubchem.inchiKey ? { inchiKey: context.pubchem.inchiKey } : {}),
    ...(context.pubchem.molecularFormula ? { molecularFormula: context.pubchem.molecularFormula } : {}),
    ...(context.pubchem.iupacName ? { iupacName: context.pubchem.iupacName } : {}),
...
export default function Home() {
  ...
  const openExpandedCanvas = useCallback(() => {
    ...
    window.requestAnimationFrame(() => expandedCanvasCloseButtonRef.current?.focus({ preventScroll: true }));
  }, []);
  ...
  const namingPubChemIdentity = useMemo(
    () => !isPristineInitialMolecule && currentMoleculeSmiles.ok
      ...
      : null,
    [activePubChemIdentity, currentCompoundContext, currentMoleculeSmiles, isPristineInitialMolecule],
  );
  const pubChemIupacName = namingPubChemIdentity?.iupacName;
  ...
  const nomenclatureVariants = (() => {
    if (externalNameIsPrimary) {
      return [{
        ...
        label: language === "en" ? "Systematic name · PubChem" : "Nombre sistemático · PubChem (inglés)",
  ...
  const activeNomenclatureConvention = !simplifiedModeEnabled
    && nomenclatureVariants.some((variant) => variant.convention === nomenclatureConvention)
  ...
  const verifiedExternalCommonName = namingPubChemIdentity
    ? verifiedPubChemCommonName(namingPubChemIdentity, language)
    : null;
  const commonNameToPresent = verifiedExternalCommonName ?? verifiedLocalCommonName;
  const visibleCommonName = commonNameToPresent
    && !nomenclatureVariants.some((variant) => normalizeNomenclatureDisplayName(variant.name) === normalizeNomenclatureDisplayName(commonNameToPresent))
    ? commonNameToPresent
    : null;
  const commonNameIsPubChemTitle = Boolean(verifiedExternalCommonName
    && currentCompoundContext?.pubchem?.recordTitle
    && normalizeNomenclatureDisplayName(verifiedExternalCommonName) === normalizeNomenclatureDisplayName(currentCompoundContext.pubchem.recordTitle));
  const commonNameLabel = verifiedExternalCommonName
    ? commonNameIsPubChemTitle
      ? language === "en" ? "PubChem record title · verified biochemical name" : "Título PubChem · nombre bioquímico verificado"
  ...
  const titleIdentity = namingPubChemIdentity ?? (currentCompoundContext?.pubchem
  ...
  const showPubChemRecordTitle = Boolean(pubChemRecordTitle && !recordTitleHasVerifiedAlias && ![
    ...
    commonNameToPresent ?? "",
    namingPubChemIdentity?.iupacName ?? "",
  ...
  const fuseSelectedBond = (size: 5 | 6) => {
    if (!selectedFusionBond) return false;
    const validationError = ringFusionError(molecule, selectedFusionBond.a, selectedFusionBond.b);
    if (validationError) {
    ...
    try {
      ...
      const committed = commit(next, language === "en" ? "Fused ring added." : "Anillo fusionado añadido.");
      if (!committed) return false;
      setFusionSelection(null);
      setShowRingPalette(false);
      return true;
  ...
  const importSmilesDocument = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const suggestedName = externalCandidateNeedsNeutralLocalName(next, importedAnalysis)
        ? null
  ...
  const ringFusionOptionError = (template: RingTemplate) => {
    ...
    if (template.kind !== "cycloalkane" || (template.size !== 5 && template.size !== 6)) {
      return language === "en"
        ...
        : "Aquí solo se pueden fusionar ciclos alifáticos de 5 o 6 miembros.";
    }
        ...
        >
          ...
          >
            ...
            <div className="panel-heading-end">
              ...
              {!canvasExpanded && <div className="history-controls" aria-label={t("Historial de cambios")}>
                <button
          ...
          >
            {canvasExpanded && (
              <div className="expanded-workspace-header">
                <strong>{t("Constructor molecular")}</strong>
                <div className="expanded-workspace-actions" aria-label={t("Historial de cambios")}>
                  <button type="button" onClick={undo} disabled={!undoStack.length} title={t("Deshacer")}>↶</button>
          ...
          {showRingPalette && (
            <div className="alkyl-palette ring-palette" id="ring-palette" ref={focusRingPicker}>
              ...
              <div className="ring-mode-switch" role="group" aria-label={t("Forma de insertar el anillo")}>
                <button
                  className={ringLibraryContext === "replace" ? "active" : ""}
                  onClick={() => {
                    setFusionSelection(null);
                    setRingInsertMode("replace");
                  }}
                  aria-pressed={ringLibraryContext === "replace"}
                >
                  <span aria-hidden="true">↻</span>
                  {t("Nueva molécula")}
                </button>
                <button
                  className={ringLibraryContext === "attach" ? "active" : ""}
                  onClick={() => {
                    setFusionSelection(null);
                    setRingInsertMode("attach");
                  }}
                ...
                <small>
                  {ringLibraryContext === "fuse"
                    ? `${selectedFusionBond?.a}–${selectedFusionBond?.b} · ${t("solo cicloalcanos de 5 o 6 miembros")}`
                    : ringLibraryContext === "attach"
                    ? language === "en"
                ...
                <div className="ring-grid cycle-ring-grid">
                  {CYCLE_TEMPLATES.map((template) => (
                    <button
                      ...
                      draggable={ringLibraryContext !== "fuse" && (template.size === 5 || template.size === 6)}
              ...
              <p className="alkyl-note ring-note">{ringLibraryContext === "fuse"
                ? t("La fusión reutiliza el enlace resaltado: los dos anillos comparten exactamente dos átomos y un enlace.")
                : t("Puedes repetir “Unir al C seleccionado” para construir moléculas con varios anillos. Los anillos quedan conectados por enlaces simples; no se fusionan ni comparten carbonos.")}</p>
            </div>
          )}
      ...
      <div className={`iupac-dock ${iupacDockExpanded ? "is-expanded" : ""}`} role="region" aria-label={t("Nombre IUPAC")}>
          ...
          <div className="iupac-dock-detail" id="iupac-dock-detail">
            ...
            {showIupacName && !simplifiedModeEnabled && (
              <div className="iupac-dock-variants" aria-label={t("Sistemas de nomenclatura disponibles")}>
                ...
                {visibleCommonName && (
                  <p className="iupac-dock-origin"><span>{commonNameLabel}</span><strong>{visibleCommonName}</strong></p>
                )}
                {namingPubChemIdentity && (namingPubChemIdentity.cid || namingPubChemIdentity.molecularFormula) && (
                  <p className="iupac-dock-origin">
                    <span>{language === "en" ? "PubChem identity" : "Identidad PubChem"}</span>
                    <strong>{[
                      namingPubChemIdentity.cid ? `CID ${namingPubChemIdentity.cid}` : "",
                      namingPubChemIdentity.molecularFormula ?? "",
                    ].filter(Boolean).join(" · ")}</strong>
                  </p>
                )}
                {namingPubChemIdentity?.iupacName && !externalNameIsPrimary && namingPubChemIdentity.iupacName !== displayedIupacName && (
                  <p className="iupac-dock-origin"><span>{language === "en" ? "Systematic name · PubChem" : "Nombre sistemático · PubChem"}</span><strong><ChemicalNameText name={namingPubChemIdentity.iupacName} /></strong></p>
                )}
        ...
        <div className="iupac-dock-inner">
          ...
          >
            ...
            {nomenclatureVariants.filter((variant) => variant.convention !== "current").map((variant) => (
          ...
          <button type="button" className="iupac-dock-copy" disabled={!showIupacName || (isPristineInitialMolecule && !showPristineMethaneName) || !displayedNameCopyable} onClick={() => {
            void copyVisibleName(displayedIupacName).then((copied) => {
              setNotice(copied
                ...
                : "No fue posible copiar el nombre al portapapeles.");
            });
          }}>{t("Copiar")}</button>
        </div>
      </div>
